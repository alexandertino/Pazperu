...
import { useActasSalidas } from '@/Composables/useActaSalidaPdf.js';

const props = defineProps({ proyecto: Object, user: Object });
const user = usePage().props.auth?.user || null;

const { actaPdf2_downloadPdfFile } = useActasSalidas();

const generarPdf = async () => {
  await actaPdf2_downloadPdfFile(form, draft, codigoGenerado, props);
};
...
const seleccionarProducto = (p) => {
  ...
  form.stock_actual = normalized.stock ?? null;
...
const stockDisponibleActual = computed(() => {
  if (form.stock_actual === null || form.stock_actual === undefined) {
    return null; // No hay información de stock
  }

  // Buscar si este producto ya está en el borrador
  const productoEnBorrador = draft.value.find(item =>
    String(item.producto_code) === String(form.producto_code) &&
    item.um === form.um
  );

  const cantidadEnBorrador = productoEnBorrador ? Number(productoEnBorrador.cantidad) : 0;
  const stockDisponible = Number(form.stock_actual) - cantidadEnBorrador;

  return Math.max(0, stockDisponible); // No puede ser negativo
});

// Computed para saber si mostrar la información de stock
const mostrarInfoStock = computed(() => {
  return form.producto_code && form.stock_actual !== null && form.stock_actual !== undefined;
});
      ...
      <div class="grid grid-cols-12 gap-6">
        ...
        <section class="col-span-8 bg-white dark:bg-gray-800 p-6 rounded shadow">
          ...
          <div class="mt-5">
            ...
            <input v-model="form.nombre_encargado" type="text" readonly class="w-full p-2 rounded bg-gray-100 dark:bg-gray-600 text-gray-900 dark:text-gray-100 
           border border-gray-300 dark:border-gray-500 focus:outline-none cursor-not-allowed" />
            <div class="mt-5"></div>
          </div>
          ...
          <div class="grid grid-cols-4 gap-3 items-end">
            ...
            <!-- Cantidad -->
            <div>
              <div v-if="mostrarInfoStock" class="mb-2">
                <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-200">
                  Stock Disponible
                </label>
                <div :class="{
                  'text-green-600 font-bold': stockDisponibleActual > 0,
                  'text-red-600 font-bold': stockDisponibleActual <= 0,
                  'text-sm': true
                }">
                  <span v-if="stockDisponibleActual > 0">
                    ✅ {{ stockDisponibleActual }} disponibles
                    <span class="text-xs text-gray-500">
                      (stock total: {{ form.stock_actual }})
                    </span>
                  </span>
                  <span v-else>
              ...
              <div v-else class="mb-2">
                ...
                <div class="text-sm text-gray-500">
                  ⏸️ Selecciona un producto para ver stock
                </div>
              </div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                Unidad: <span class="font-semibold text-indigo-600">{{ form.um || '—' }}</span>
              </p>
              ...
              <div v-if="mostrarInfoStock && form.cantidad > stockDisponibleActual" class="text-xs text-red-600 mt-1">
